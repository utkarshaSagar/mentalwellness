...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'checkin' | 'journal' | 'timer' | 'breathing' | 'settings' | 'eyecalm' | 'diagnostics'>('dashboard');
          ...
          <div className="bg-white border border-stone-100 rounded-3xl p-5 shadow-sm space-y-4">
            ...
            <nav id="desk-navigation" className="flex flex-col space-y-1">
              {([
                ...
                { id: 'diagnostics', label: "Run Verification Checks", icon: BadgeAlert }
              ] as const).map((tab) => {
                const Icon = tab.icon;
